...
import os
import hanja
from source.TextProcessing.KoG2P import runKoG2P
...
hangul_symbol_1 = hangul_symbol_1 + special_symbol
...
def hangul_to_sequence(dir, hangul_text, hangul_type=1):
    ...
    rulebook = os.path.join(dir, 'source/TextProcessing/rulebook.txt')
    hangul_text = number_to_hangul(hangul_text)
    hangul_text = hangul_text + u"␃"  # ␃: EOS
    hangul_text = hanja.translate(hangul_text,'substitution')
...
def sequence_to_string(dir, sequence):
    text = [ids_to_hangul[id] for id in sequence]
...
def number_to_hangul(text):
    ...
    for strNum in numbers:
        if len(strNum) <=9:
            number_text = digit2txt(strNum)
            text = text.replace(strNum, number_text, 1)
        else:
            for num in strNum:
                number_text = digit2txt(num)
                text = text.replace(num, number_text, 1)
    return text
