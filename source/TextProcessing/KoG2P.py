...
def isHangul(charint):
    ...
    hangul_init_3=4352
    hangul_fin_3=4546
    if (charint >= hangul_init_1 and charint <= hangul_fin_1):
        return True
    elif (charint >= hangul_init_2 and charint <= hangul_fin_2):
        return True
    elif (charint >= hangul_init_3 and charint <= hangul_fin_3):
        return True
    else:
        return False
...
def eng_2_kor(text):
    ko = ['ㅂ','ㄲ','ㄷ','ㅆ','ㄱ','ㅎ','ㅉ','ㅋ','ㄹ','ㅁ','ㄴ','ㅍ','ㅋ','ㅈ','ㅅ','ㅌ','ㅃ','ㄸ','ㅙ','ㅏ','ㅓ','ㅣ','ㅗ','ㅜ','ㅘ','ㅟ']
    en=['b','c','d','f','g','h','j','k','l','m','n','p','q','r','s','t','v','x','z','a','e','i','o','u','w','y']
    for id, char in enumerate(text):
        if char.lower() in en:
            id_in_en = en.index(char.lower())
            text=text.replace(char, ko[id_in_en])
    return text


def detect_special_symbols(text):
    for id, char in enumerate(text):
        if char.lower().encode('ascii', 'ignore') in range(128, 256):
            print(char)
    return text
